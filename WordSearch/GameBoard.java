...
public class GameBoard {
    ...
    public String isWord(String word) {
        ...
        for(int i =0; i<n; i++)
        {
            for(int j = 0; j<n;j++)
            {
                if(findPath(wordtoFind, pathSoFar, i, j))
                {
                    return true;
                }
            }
    ...
    private boolean findPath(String wordToFind, 
                             ...
                             int row, int col)
    {
       //Base case is if the word is a single character then it can be found on the boards
        //we need to make a char for the potention first character
        int index = 0;
        char first_char = wordToFind.charAt(index);

        //we need to insure that we do a full board search for the first char
        if(pathSoFar.isEmpty() == true)
        {
            if(board[row][col] == first_char)
            {
                //we want to now add by pushing onto the stack
                pathSoFar.push(row + ", " + col);
              
                //we want to call the method however we now want to look for the second character
                //not sure why there was an error while calling the method.
                //findPath(String wordToFind.charAt(index+1), Stack<String> pathSoFar,row, col)
            }
        }
