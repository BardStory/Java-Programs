...
public class GameBoard {
	...
	Stack <string> pathSoFar = new Stack();
    ...
    public String isWord(String word) {
      char letter = word.subString(0,1);
                             ...
                             int row, int col)
    {
		...
		char letter = wordToFind.subString(0,1);
		
		
		if (pathSoFar.isEmpty() == true){ //ensures that only the initial substring prompts a full gameboard search for the initial token
			if (board[row][col] == letter){
				pathSoFar.push(row +", "+ col);
				findPath(String wordToFind.subString(pathSoFar.length()), Stack<String> pathSoFar, int row, int col); //start the adjacency loop 
			}else					 //keep looking for the begining of the letter 
				for (int i = row; i <  NUM_ROWS; i++){ //this will be redundant once adjacency is written, replace
					for (int j = col; j < NUM_COLS; j++){
						findPath(String wordToFind, 
                             Stack<String> pathSoFar,
                             int i, int j) 
					}
				} 
		}/*else 	//first letter found, pushed, adjacency begins HOW DOES VISITED WORK WITH THIS?
			try 
				switch (board[row][col])
					case 1: board[row][col] == wordToFind.subString(pathSoFar.length());
							break;
					case 2: board[row+1][col+0] == wordToFind.subString(pathSoFar.length());
							break;		
					case 3: board[row+0][col+1] == wordToFind.subString(pathSoFar.length());
							break;
					case 4: board[row+1][col+1] == wordToFind.subString(pathSoFar.length());
							break;
					case 5: board[row-1][col+0] == wordToFind.subString(pathSoFar.length());
							break;
					case 6: board[row+0][col-1] == wordToFind.subString(pathSoFar.length());
							break;
					case 7: board[row-1][col+1] == wordToFind.subString(pathSoFar.length());
							break;
					case 8: board[row+1][col-1] == wordToFind.subString(pathSoFar.length());
							break;
					case 9: board[row-1][col-1] == wordToFind.subString(pathSoFar.length());
							break;
				
			catch outOfBoundsException.java // check this 
